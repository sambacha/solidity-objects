...
class TruffleObjectMapper {
	...
	methodNameToPropertyName(methodName) {
		return 0 === methodName.indexOf("get") ? camelCase(methodName.substring(3)) : methodName;
	}

	getFilteredNodeList(contractName) {
		const artifact = this.getArtifact(contractName);
		return artifact.abi.filter(node => "function" === node.type && "view" === node.stateMutability && !node.inputs.length);
	}
